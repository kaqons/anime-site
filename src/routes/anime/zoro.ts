...
const routes = async (fastify: FastifyInstance, options: RegisterOptions) => {
  ...
  // Spotlight endpoint temporarily guarded due to upstream provider instability
  fastify.get('/spotlight', async (_: FastifyRequest, reply: FastifyReply) => {
    try {
      const res = await zoro.fetchSpotlight();
      reply.status(200).send(res);
    } catch (error) {
      reply.status(503).send({ message: 'Spotlight temporarily unavailable. Please try again later.' });
    }
  });
